...
def cube(points_list_2d, nvx_face, div, ordre, couleur,res_fenetre,fenetre):
    """dessine un cube 3d """
    c = Canvas(fenetre,width=res_fenetre[0], height=res_fenetre[1], bg='white')
    c.pack()
    ...
    for h in ordre: #permet de dessiner 6 faces
        ...
        c.create_polygon(nvx_coord, fill=couleur[h])
    c.update()
    c.destroy()
...
def affichage(points_list, face, couleur, choix_3d, div, res_fenetre,fenetre):
    ...
    while tour <= fps:
        ...
        cube (points_list_2d, nvx_face, div, ordre, couleur_fps, res_fenetre, fenetre)
