...
def in2d_Thales(points_list, camera,rapprochement,mur):
    ...
    for i in points_list:
        ...
        nvx_points_list.update( {i: (round(rapport*(points_list[i][1]-camera[1])+camera[1], 12),
                                round(rapport*(points_list[i][2]-camera[2])+camera[2], 12))} )
    return nvx_points_list


def in2d_parameter(points_list, camera, rapprochement, mur):
    """transformes the coordinates of the points from 3d to 2d
    the camera coordinates are in variable and the wall is 'mur'
    ...
    for i in points_list:
        ...
        point_projection={ i: (round(camera[1]+vector_plan_y ,12),
...
def separation_face(face,points_list,div):
    ...
    nvx_face=[]
    tt_points={}
    lettre_face=('a','b','c','d','e','f','g','h','i','j','k','l')
    indice_lettre=0
    for i in face:
        ...
        nvx_face.extend(make_face(letter,div))
        tt_points.update(temp_points)
        indice_lettre+=1

    return nvx_face, tt_points


def face_color(div):
    """renvoie une liste de couleur"""
    list_color=[]
    for i in range( (div+1)**2*6 ):
...
def affichage(points_list,face,couleur,choix_3d,div):
    ...
    while tour <= fps:
        camera = (50*10+rapprochement, 0, 0)
        mur = (50*2+rapprochement,0,0)  # le mur est sur l'axe des x
        
        nvx_face, nvx_points_list = separation_face(face, points_list,div)
        print(nvx_points_list)
        ordre = dist_camera_faces(nvx_points_list, nvx_face, camera)
        
        if choix_3d == 0:
            points_list_2d = in2d_Thales(nvx_points_list, camera,rapprochement,mur)
        else:
            points_list_2d = in2d_parameter(nvx_points_list, camera,rapprochement,mur)
